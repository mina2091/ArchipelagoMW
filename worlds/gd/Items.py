...
def create_gd_items(world):
    ...
    for i in range( world.options.level_amount ):
        ...
        for j in range(world.options.checks_per_level):
            world.itempool.append(Item(item_name, ItemClassification.progression, world.item_name_to_id[item_name], world.player))
    for i in range(total_location_count - len(world.itempool)):
        world.itempool.append(Item("Filler", ItemClassification.filler, world.item_name_to_id["Filler"], world.player))
